...
void Shader::init(const std::string& vertFile, const std::string& fragFile)
{
	...
	// Test code
	//glBindAttribLocation(program, 3, "FragColor");
	//glBindAttribLocation(program, 4, "BrightColor");
...
void Shader::initGeo(const std::string& vertFile, const std::string& geomFile, const std::string& fragFile)
{
	...
	shadersGeo[1] = CreateShader(LoadShader(geomFile), GL_GEOMETRY_SHADER); // create vertex shader
	shadersGeo[2] = CreateShader(LoadShader(fragFile), GL_FRAGMENT_SHADER); // create fragment shader

	for (unsigned int i = 0; i < NUM_SHADERS_GEO; i++)
	{
